...
function PracticeRoute() {
  ...
  const [solvedIds, setSolvedIds] = useState<Set<number>>(() => {
    ...
    allWords.forEach(w => {
      if (w.revealed) {
        // auto-solve all identical words across the entire paragraph
        allWords.forEach(ww => {
          if (ww.ar === w.ar) {
            initial.add(ww.id)
          }
  ...
  useEffect(() => {
    if (solvedIds.size === allWords.length && allWords.length > 0) {
      confetti({
        particleCount: 100,
        spread: 70,
        origin: { y: 0.6 }
      })
      // TODO: Play success audio file
  ...
  return (
    <main className="min-h-screen bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100 pb-24 font-sans selection:bg-neutral-900 selection:text-white dark:selection:bg-white dark:selection:text-neutral-900 transition-colors duration-300">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-12">
        <CurriculumHeader 
          volume={volNum} 
          lesson={darsNum} 
          steps={practiceSteps} 
        ...
        <div className="w-full bg-neutral-200 dark:bg-neutral-700 h-2.5 rounded-full mb-12 flex overflow-hidden">
          {allWords.map((w, i) => (
            <div 
              key={`progress-${w.id}-${i}`}
              className={`flex-1 border-r border-white/30 dark:border-neutral-800/30 last:border-0 transition-colors duration-500 ${
                solvedIds.has(w.id) ? 'bg-primary-500' : 'bg-transparent'
              }`}
            />
          ))}
        </div>
        ...
        {/* Paragraph Display */}
        <div 
          className="bg-neutral-100 dark:bg-neutral-800/80 p-4 sm:p-5 rounded-[2.5rem] mb-8"
          dir="rtl"
        >
        ...
        <div className="bg-neutral-100 dark:bg-neutral-800/80 p-4 sm:p-5 rounded-[2.5rem]">
          <div className="bg-white dark:bg-neutral-900 p-6 sm:p-8 rounded-[2rem] flex flex-col gap-4 shadow-sm border border-transparent">
            <h3 className="text-xs font-english-bold text-neutral-500 dark:text-neutral-400 mb-2 uppercase tracking-wider text-center">Translation</h3>
                  ...
                  if (typeof part === 'string') {
                    ...
                    return split.map((s, i) => {
                      if (i % 2 === 1) { // This is the match
                        ...
                        return <span key={`blank-${w.id}-${lineIndex}-${partIndex}-${i}`} className="inline-block h-5 bg-neutral-200 dark:bg-neutral-700 rounded-md mx-1 align-middle animate-pulse" style={{ width: `${len * 0.6}rem` }} />;
                      }
                      return s;
                    });
                  }
        ...
        {solvedIds.size === allWords.length && (
          ...
          {/* Navigation */}
          <div className="mt-8 pt-6 border-t border-neutral-200/60 dark:border-neutral-700/50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <button
              onClick={() => navigate({ to: '/practice/$volumeId/$lessonId', params: { volumeId: `vol${volNum}`, lessonId: `lesson${darsNum > 1 ? darsNum - 1 : 1}` } })}
              disabled={darsNum === 1}
            ...
            <button
              onClick={() => navigate({ to: '/practice/$volumeId/$lessonId', params: { volumeId: `vol${volNum}`, lessonId: `lesson${darsNum < 9 ? darsNum + 1 : 9}` } })}
