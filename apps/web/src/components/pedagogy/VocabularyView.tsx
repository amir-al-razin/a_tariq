...
{done
 ...
 disabled={done}
 className="w-full min-h-[200px] rounded-3xl overflow-hidden transition-all active:scale-[0.98] outline-none"
 >
{!flipped ? (
 <div className="w-full h-full min-h-[200px] bg-white dark:bg-neutral-950 flex flex-col items-center justify-center p-6">
{word.imageUrl ? (
...
{words.map((_, i) => (
 ...
 i < currentIndex || done ? '' : i === currentIndex ? '' : 'bg-neutral-200 dark:bg-neutral-800'
}`}
...
{done ?`${total} / ${total}`:`${currentIndex + 1} / ${total}`}
 ...
 className={`flex-1 h-12 rounded-3xl flex items-center justify-center gap-1.5 transition-all outline-none ${
 (isFirst && !flipped)
