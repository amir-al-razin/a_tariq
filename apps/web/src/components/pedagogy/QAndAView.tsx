...
{/* Emoji */}
 <div className="w-24 h-24 rounded-3xl bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center mb-5 overflow-hidden">
{q.imageUrl ? (
...
{q.options_ar.map((opt) =>{
 ...
 disabled={revealed}
 className={`p-4 rounded-3xl -2 flex items-center justify-center transition-colors cursor-pointer ${
 !revealed ? 'hover:bg-neutral-50 dark:hover:bg-neutral-900' : ''
} ${borderColor} ${bg}`}
 >
...
{revealed && (
 <div
 className={`mt-4 p-3.5 rounded-3xl w-full ${
 isCorrect
...
{q.explanation && (
 <div className="mt-3 pt-3 -black/10 dark:-white/10 font-english text-sm text-neutral-700 dark:text-neutral-300">
...
{revealed && !isLastQ && (
 <button
 onClick={handleNext}
 className="mt-5 px-8 py-3.5 rounded-3xl transition-colors flex items-center justify-center w-full cursor-pointer"
