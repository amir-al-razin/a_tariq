 ...
 * Powers Real-Time Quran Mushaf Illumination (/mushaf-v2) and Lexical Vault (/words).
 */

// Regex for Arabic diacritical marks (Tashkeel / Harakat / Quranic annotation glyphs, excluding dagger alif which is handled semantically)
...
export function stripHarakat(text: string): string {
  if (!text) return '';
  // Remove dagger alif as vowel mark in basic strip
  return text.replace(/\u0670/g, '').replace(HARAKAT_REGEX, '').trim();
}
 ...
 */
export function buildKnownTokensSet(items: Array<{ arabic: string; lemma?: string }>): Set<string> {
...
export function calculatePageComprehension(
  words: Array<{ text_uthmani: string; char_type_name: 'word' | 'end' }>,
