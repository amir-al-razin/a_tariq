...
export const LessonNode: React.FC<Props> = ({
  number, status, isDark, faceColor, shadowColor,
  textColor, entryDelay = 0, onPress,
}) => {
  const SIZE = 72;
  ...
  return (
    ...
    >
      ...
      >
        {({ pressed }) => {
          const pushDepth = pressed && !isLocked ? 0 : -6;
          return (
            <View style={{ width: SIZE, height: SIZE + 6, justifyContent: 'flex-end' }}>
              <View style={{ position: 'absolute', bottom: 0, width: SIZE,
                height: SIZE + (pressed && !isLocked ? 0 : 6),
                borderRadius: SIZE / 2, backgroundColor: shadowColor }} />
              <View style={{ width: SIZE, height: SIZE, borderRadius: SIZE / 2,
                backgroundColor: faceColor, alignItems: 'center',
                justifyContent: 'center', transform: [{ translateY: pushDepth }] }}>
                {isLocked
                  ...
                  : <Text style={{ fontFamily: 'Lexend_600SemiBold',
                      fontSize: 24, color: textColor, lineHeight: 30 }}>{number}</Text>
                }
              </View>
            </View>
          );
