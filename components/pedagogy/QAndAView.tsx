...
type PedagogyProps = {
  isDark: boolean;
  C: ColorMap;
  payload?: { questions?: QAItem[]; instruction?: string; instructionBn?: string; text?: string };
  onProgress?: (v: number) => void;
  onComplete?: () => void;
};

export const QAndAView: React.FC<PedagogyProps> = ({
  isDark,
  C,
  payload,
  onProgress,
  ...
  if (questions.length === 0) {
    ...
    return (
      <Text style={{ color: isDark ? C.neutral100 : C.neutral800 }}>{t('qanda.noQuestions')}</Text>
    );
  }
